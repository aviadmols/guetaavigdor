 ...
 *
 * Gueta: the theme's copy. A thin orange line runs along the top of the card,
 * and the logo is the theme's own PNG, centred, instead of the image in
 * WooCommerce's email settings, which still held the previous site's logo.
 * The rest is WooCommerce's, so its hooks and structure stay as they are. The
 * styles are in inc/gueta-emails.php.
 *
...
$gueta_logo = function_exists( 'gueta_email_logo_url' ) ? gueta_email_logo_url() : '';
							...
							<tr>
								<td align="center" valign="top">
									<table border="0" cellpadding="0" cellspacing="0" width="100%" role="presentation">
										<tr>
											<td class="gueta-email-accent" height="4">&nbsp;</td>
										</tr>
									</table>
									<?php
									$img = $gueta_logo ? $gueta_logo : get_option( 'woocommerce_email_header_image' );
									/**
									 * This filter is documented in templates/emails/email-styles.php
									 *
									 * @since 9.6.0
									 */
									if ( ! $gueta_logo && apply_filters( 'woocommerce_is_email_preview', false ) ) {
										...
										<table border="0" cellpadding="0" cellspacing="0" width="100%" role="presentation">
											<tr>
												<td id="template_header_image">
													<?php
													if ( $img ) {
														$image_html = '<img src="' . esc_url( $img ) . '" alt="' . esc_attr( $store_name ) . '"' . ( $gueta_logo ? ' width="220" height="26"' : '' ) . ' />';
														if ( $header_image_url ) {
